...
//
// Le label sono job (nome del job di config) e type (tipo di task/WorkItem). Non usare mai
...
func TaskStart(taskType string) time.Time {
	TaskStarted.WithLabelValues(taskType).Inc()
	return time.Now()
}

// ObserveTask emette l'esito e la durata di un task concluso, su qualunque percorso.
func ObserveTask(taskType string, o store.Outcome, started time.Time) {
	TaskOutcome.WithLabelValues(taskType, OutcomeName(o)).Inc()
	TaskDuration.WithLabelValues(taskType, Status(o)).Observe(time.Since(started).Seconds())
}
...
func JobClaimed(job, taskType string, n int) {
	if n <= 0 {
		return
	}
	JobItemsClaimed.WithLabelValues(job, taskType).Add(float64(n))
}
...
func JobProcessed(job, taskType string, o store.Outcome, started time.Time) {
	status := Status(o)
	JobItemsProcessed.WithLabelValues(job, taskType, status).Inc()
	JobItemsProcessedDuration.WithLabelValues(job, taskType, status).Observe(time.Since(started).Seconds())
}

// TaskAssigned registra l'esito di un dispatch: err nil è un'assegnazione riuscita.
func TaskAssigned(taskType string, err error) {
	status := StatusSuccess
	if err != nil {
		status = StatusError
	}
	TaskAssignedTotal.WithLabelValues(taskType, status).Inc()
}

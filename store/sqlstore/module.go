...
func ModuleIf(modes ...string) {
	core.ProvidesIf(fx.Annotate(newBatchDataSQL, fx.As(new(store.IData))), modes...)
	core.ProvidesIf(fx.Annotate(newWorkItemDataSQL, fx.As(new(store.IWorkItemStore))), modes...)
}
